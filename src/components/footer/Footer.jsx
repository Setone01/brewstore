...
import twitter from "../../assets/svg/facebook.svg";
// import facebook from "../../assets/svg/facebook.svg";
// import tiktok from "../../assets/svg/facebook.svg";
// import instagram from "../../assets/svg/facebook.svg";

const Footer = () => {
  return (
    <div className="w-screen max-h-max bg-black pt-[60px] pb-8 mt-auto">
      <div className={`w-[90%] mx-auto flex justify-center flex-col`}>
        <div className="w-full flex justify-between xs:flex-col md:flex-row">
          <div className=" basis-1/3 mb-6">
            <div className=" mb-8">
              <img src={logo2} alt="" />
            </div>

            <p className=" text-[#CFCFCF] text-sm font-normal">
          ...
          <div className="">
            <LinkGroup header={"Services"}>
              <NavLink link={"/"} label={"Bonus program"} />
              <NavLink link={"/"} label={"Gift cards"} />
              <NavLink link={"/"} label={"Credit and payment"} />
              <NavLink link={"/"} label={"Service contracts"} />
              <NavLink link={"/"} label={"Non-cash account"} />
              <NavLink link={"/"} label={"Payment"} />
            </LinkGroup>
          </div>
          <div className="">
            <LinkGroup header={"Assistance to the buyer"}>
              <NavLink link={"/"} label={"Find an order"} />
              <NavLink link={"/"} label={"Terms of delivery"} />
              <NavLink link={"/"} label={"Exchange and return of goods"} />
              <NavLink link={"/"} label={"Guarantee"} />
              <NavLink link={"/"} label={"Frequently asked questions"} />
              <NavLink link={"/"} label={"Terms of use of the site"} />
            </LinkGroup>
          </div>
        </div>
...
const LinkGroup = ({ children, header }) => {
  return (
    <>
      <div className="w-full">
        <div className=" mb-5">
          <h4 className=" text-lg text-white font-semibold">{header}</h4>
          <ul>{children}</ul>
        </div>
      </div>
    </>
  ...
  return (
    <div>
      <li>
        <a
          href={link}
          className={`inline-block text-sm text-[#CFCFCF] font-light leading-loose  hover:text-white`}
        >
          {label}
        </a>
      </li>
